...
class EzGurobi:
    ...
    def ezload(self, filename):
        ...
        if len(self.cons_df.columns) == self.n_variables +2:
            self.cons_df.columns = np.append(self.variable_name,['constraint type','RHS values'])
        elif(len(self.cons_df.columns) == self.n_variables + 3):
            self.cons_df.columns = np.append(self.variable_name,['constraint type','RHS values','constraint name'])
        else:
            raise KeyError("Wrong number of constraints")
    ...
    def ezrun(self,obj='min', run_sa = False):
        ...
        if self.cons_df.shape[0]:
            for row in self.cons_df.itertuples(): 
                symbol = row[self.n_variables +1]
                if symbol =='>=':
                    m.addConstr(quicksum(float(row[j+1])* x[j] for j in range(self.n_variables)) >= float(row[self.n_variables + 2]))
                elif symbol =='<=':
                    m.addConstr(quicksum(float(row[j+1])* x[j] for j in range(self.n_variables)) <= float(row[self.n_variables + 2]))
                elif symbol =='=':
                    m.addConstr(quicksum(float(row[j+1])* x[j] for j in range(self.n_variables)) == float(row[self.n_variables + 2]))
                else:
                    raise ValueError("Wrong Constraint Type as {}".format(symbol))
